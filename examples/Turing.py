...
class TuringTrainer(PINNTrainer):
    def _test_step(self, all_params, i):
        ...
        x_batch_test = self.domain.get_test_points(all_params, self.n_test)
        ...
        fig = plot_turing_patterns(x_batch_test, u_test, v_test, 
                                 t_idx=self.n_test[2]//2,  # middle time point
                                 n_test=self.n_test)
...
# After training, generate evolution plot
x_batch_test = run.domain.get_test_points(run.all_params, run.n_test)
uv_test = run.network.apply(run.all_params, x_batch_test)
u_test, v_test = uv_test[:, 0], uv_test[:, 1]

figs = plot_turing_evolution(x_batch_test, u_test, v_test, run.n_test)
plt.show()